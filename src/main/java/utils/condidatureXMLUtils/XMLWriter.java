...
public class XMLWriter {
    public static void appendXML(CondidatureListXML jobOfferList, File xmlFile) throws Exception {
        ...
        if (!xmlFile.exists()) {
            ...
            jobOfferList.setCondidatures(List.of());  // empty list if no offers are present initially
        ...
        for (CondidatureXML newOffer : jobOfferList.getCondidatures()) {
    ...
    public static void writeXML(CondidatureListXML jobOfferList, File xmlFile) throws Exception {
        JAXBContext context = JAXBContext.newInstance(CondidatureListXML.class);
        Marshaller marshaller = context.createMarshaller();
        marshaller.setProperty(Marshaller.JAXB_FORMATTED_OUTPUT, true);
        marshaller.marshal(jobOfferList, xmlFile);
